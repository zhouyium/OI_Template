...
public:
    ...
    LazySegmentTreeWithSetUpdate(vector<T> & v){
        int n=v.size();
        init(n);
        for(int i=0;i<n;i++){
            arr[i]=v[i];
    ...
    LazySegmentTreeWithSetUpdate(T v[],int n){
        init(n);
        for(int i=0;i<n;i++){
            arr[i]=v[i];
        }
    ...
    static void how_to_use(){
        ...
        LazySegmentTreeWithSetUpdate<int> *mylst=new LazySegmentTreeWithSetUpdate(arr);
        mylst->update(0,3,0);
        mylst->update(2,3,2);
        mylst->update(0,2,10);
